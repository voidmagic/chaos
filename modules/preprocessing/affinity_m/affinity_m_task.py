...
@register_task("affinitym_task")
class AffinityMTask(MultilingualTranslationTask):
    ...
    def get_random_batch(self):
        ...
        valid_key = random.choice(list(self.validation_batches.keys()))
        return valid_key, random.choice(self.validation_batches[valid_key])
    ...
    def calculate_affinity(self, model, criterion):
        ...
        if self.last_valid_sample is not None:
            ...
            _, _, logging_output = criterion(model.models[self.last_valid_key], self.last_valid_sample)
    ...
    def train_step(self, sample, model, criterion, optimizer, update_num, ignore_grad=False):
        ...
        # 选择一个语言训练，其他的语言为None
        self.last_train_key = random.choice(list(sample.keys()))
        sample = {key: value if key == self.last_train_key else None for key, value in sample.items()}
        loss, sample_size, logging_output = super(AffinityMTask, self).train_step(sample, model, criterion, optimizer, update_num, ignore_grad)
