...
bilingual_prop_path = "/home/qwang/030-transfer/003-models/401-bilingual-iwslt/en-zh/zh.prop"
eszh_prop_path = "/home/qwang/030-transfer/003-models/400-multilingual-iwslt/eszh-o2m/zh.prop"
all_prop_path = "/home/qwang/030-transfer/003-models/400-multilingual-iwslt/o2m/zh.prop"
...
all_dict = parse_file(all_prop_path)
...
all_loss = [float(v) for value in all_dict.values() for v in value["loss"].split()]
...
print("all loss\t  ", -sum([math.log(v+1e-7) for v in all_loss]) / len(bi_loss))

print()

print("bilin mean prop\t", sum(bi_loss) / len(bi_loss))
print("eszh mean prop\t ", sum(eszh_loss) / len(bi_loss))
print("all mean prop\t  ", sum(all_loss) / len(bi_loss))
...
print("ideal all loss  \t", -math.log(sum(all_loss) / len(bi_loss)))
...
combine_dict = {
    key: {
        ...
        "losses": {
            "bi": [float(v) for v in bi_dict[key]["loss"].split()[:-1]],
            "eszh": [float(v) for v in eszh_dict[key]["loss"].split()[:-1]],
            "all": [float(v) for v in all_dict[key]["loss"].split()[:-1]],
        },
    }
...
for key, value in combine_dict.items():
    for word, bil, eszhl, al in zip(value['target'], value['losses']['bi'], value['losses']['eszh'], value['losses']['all']):
        if eszhl > bil:
            counter[word] += 1
        elif bil > eszhl:
            counter[word] -= 1
